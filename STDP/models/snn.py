...
class STDP(nn.Module):
    ...
    def forward(self, input_spikes, potentials, output_spikes, winners, kwta=1, inhibition_radius=0):
        ...
        for i, (winner,) in enumerate(winners):
            pair = pairings[i].detach().clone().to(device)
            lr0, lr1 = self.learning_rate.clone().detach().to(device)
            winner_lr = torch.index_select(lr, 0, winner)
            winner_lr[:] = torch.where(pair, lr0, lr1)
