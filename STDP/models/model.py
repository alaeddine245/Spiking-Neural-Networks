...
class Network(nn.Module):
    ...
    def __init__(self, device='cuda'):
        ...
        self.conv2 = snn.Convolution(
            in_channels=30, out_channels=100, kernel_size=2)
        ...
        if self.training:
            ...
            if layer_idx == 1:
                ...
                if self.spk_cnt >= 500:
                    self.spk_cnt = 0
                    ap = torch.tensor(
                        self.stdp1.learning_rate[0].item(), device=self.device) * 2
            ...
            if self.inference_cnt % 10000 == 0:
                print(f'Inference {self.inference_cnt} images')
    ...
    def stdp(self, layer_idx):
        if layer_idx == 1:
            self.stdp1(self.ctx["input_spikes"], self.ctx["potentials"],
                       self.ctx["output_spikes"], self.ctx["winners"], self.conv1_kwinners, self.conv1_inhibition_rad)
        else:
            self.stdp2(self.ctx["input_spikes"], self.ctx["potentials"],
                       self.ctx["output_spikes"], self.ctx["winners"], self.conv2_kwinners, self.conv2_inhibition_rad)
